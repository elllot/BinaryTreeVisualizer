...
class BTreeVisualizer:
    ...
    " by the midpoint.
    """
    def fill_tree_grid(self, root, grid, lv, i, j):
        if root:
            m = i + (j-i) // 2 # mid point
            grid[lv][m] = str(root.val); lv += 1 # lv ++
            self.fill_tree_grid(root.left, grid, lv, i, m-1) # left child
            self.fill_tree_grid(root.right, grid, lv, m+1, j) # right child
    ...
    def visualize(self, root):
        ...
        self.fill_tree_grid(root, grid, 0, 0, width-1)
